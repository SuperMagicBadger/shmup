...
public class MainMenuScreenOld implements Screen {
	...
	public MainMenuScreenOld(SHMUP game) {
		// INIT VARIABLES
		this.game = game;
		uiStage = new Stage();
		...
		highScores.addListener(new ClickListener() {
			...
			public void clicked(InputEvent event, float x, float y) {
				scoreTable.clear();
				for (Score s : MainMenuScreenOld.this.game.score.highScores) {
					Label l = new Label(s.toString(),
							MainMenuScreenOld.this.game.skn, "font_25",
							Color.WHITE);
		...
		settings.addListener(new ClickListener(){
			public void clicked(InputEvent event, float x, float y) {
				MainMenuScreenOld.this.game.setScreen(SettingsScreen.screenTag, screenTag);
			}
		});
