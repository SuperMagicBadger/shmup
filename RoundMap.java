...
//		 load("round_config_one");
	...
	public void load(String levelFile) {
		...
		try {
			Element root = reader.parse(Gdx.files.internal("levels/"
					+ levelFile + ".xml"));
			...
			for (Element e : root.getChildrenByName("block")) {
				...
				if (e.getChildByName("sprite") != null) {
					...
					if (e.getChildByName("pos") != null) {
						...
						x = Float.parseFloat(spPos.substring(0,
								spPos.indexOf(':')));
						y = Float
								.parseFloat(spPos.substring(spPos.indexOf(':') + 1));
					}
					if (e.getChildByName("rot") != null) {
						rot = Float.parseFloat(e.getChildByName("rot")
								.getText());
					}
					if (e.getChildByName("size") != null) {
						String spSze = root.getChildByName("size").getText();
						w = Float
								.parseFloat(s.substring(0, spSze.indexOf(':')));
						h = Float
								.parseFloat(s.substring(spSze.indexOf(':') + 1));
					}
				...
				if (gs != null) {
					...
					System.out.println("rot: " + gs.getRotation() + " - ("
							+ gs.getX() + ", " + gs.getY() + ")");
	...
	public void spawnNextLevel() {
		...
		int idiotCount = getIdiotCount();

		float index;
		
		for (int i = 0; i < idiotCount; i++) {
			index = i / (float) idiotCount;
			spawnIdiotBro(spawnX(index), spawnY(index));
		}
		index = SHMUP.rng.nextFloat();
...
//		return Math.min(MAX_ENEMIES, level * 2 + 2);
//	}
	
	private int getIdiotCount() {
		return Math.min(level + 5, MAX_IDIOT_BROS);
	}

	private int getDashCount() {
		if(level < DASH_BRO_LEVEL){
			return 0;
		}
		return ((level - DASH_BRO_LEVEL + 1)) % MAX_DASH_BROS;
	}
